...
async def verify_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if hasattr(db, 'is_task_completed') and db.is_task_completed(user.id, task_id):
...
def setup_app(token):
    ...
    app.add_handler(CallbackQueryHandler(reject_cb, pattern="^reject_"))
